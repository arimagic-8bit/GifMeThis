...
const Favourites = () => {
  const favourites = useSelector((reduxState) => Object.values(reduxState.gifs.favourites));

  useEffect(() => {
    const retrieved = retrieveFromLocalStorage();
    if (!retrieved.length) {
      const instanced = {};
      Object.entries(retrieved).forEach(([key, value]) => {instanced[key] = new Media(value) });
